...
export function RankingTable({ rows, year, selectedId, onSelect }: {
  ...
  return <section className={styles.panel} aria-labelledby="ranking-table-title">
    ...
    <table className={styles.table}>
      <caption>{year}年末 · 対象20社内の順位 · 時価総額（十億米ドル）</caption>
      ...
      <tbody>{rows.map(row => <tr key={row.id} data-selected={selectedId === row.id}>
        <td>{row.rank}</td><th scope="row"><button type="button" onClick={() => onSelect(row.id)} aria-pressed={selectedId === row.id} aria-controls="ranking-company-detail">{row.displayName}</button></th>
