  ...
  return <section className={styles.panel} aria-labelledby="ranking-table-title">
    ...
    <table className={styles.table}>
      ...
      <tbody>{rows.map(row => <tr key={row.id} data-selected={selectedId === row.id}>
        <td>{row.rank}</td><th scope="row"><button type="button" onClick={() => onSelect(row.id)} aria-pressed={selectedId === row.id} aria-controls="ranking-company-detail">{row.displayName}</button><span className={styles.small}>{rows.some(company => company.isReference) ? (row.isReference ? '比較対象（他業界）' : '半導体・装置') : row.category}</span></th>
