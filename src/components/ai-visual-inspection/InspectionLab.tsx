...
export function InspectionLab() {
  ...
  return <section className={styles.lab} aria-label="AI外観検査の操作画面">
    ...
    {step === 2 && <section className={styles.training}><label>この例で事前学習したモデルに切り替える<select value={settings.model} onChange={e => { const model = e.target.value as ModelId; setSettings(s => ({ ...s, model })); event("model_change", model); }}>{MODEL_IDS.map(id => <option key={id} value={id}>{inspectionModels[id]}</option>)}</select></label>
    ...
    {sample && current && <>
      ...
      <label>画像一覧<select value={filter} onChange={e => setFilter(e.target.value as Filter)}><option value="all">すべて</option><option value="missed">見逃した画像（どちらかの方式）</option><option value="rejected">はじいた良品（どちらかの方式）</option></select></label>
    ...
    {step === 4 && locked && <section className={styles.reflection}><h3>この条件なら、どう採用しますか？</h3><p>ルール：設定と撮像の調整。AI：画像収集・領域ラベル作成・学習後の検証。併用：両方の準備と検証が必要です。</p><label>採用候補<select value={choice} onChange={e => { setChoice(e.target.value); setFinished(false); }}><option value="">選んでください</option>{["ルール", "AI", "併用", "追加検証"].map(v => <option key={v}>{v}</option>)}</select></label><label>重視したこと<select value={reason} onChange={e => { setReason(e.target.value); setFinished(false); }}><option value="">選んでください</option>{["見逃しを抑える", "良品をはじきすぎない", "設定・データ準備の負担", "撮影条件への対応"].map(v => <option key={v}>{v}</option>)}</select></label><button disabled={!choice || !reason || !results} onClick={() => { setFinished(true); event("finish", "reflection"); }}>この理由で振り返りを終える</button>{finished && <p role="status">「{choice}」を「{reason}」から選びました。一律の正解はありません。撮像条件と必要な検証を現場ごとに考えましょう。</p>}<p><Link href="/tools/gage-rr" onClick={() => event("related", "gage_rr")}>次はGage R&amp;Rで、連続値の測定ばらつきを体験 →</Link></p></section>}
