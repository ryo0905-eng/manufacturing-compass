...
type ToolId = "gage-rr" | "control-chart" | "cpk" | "doe" | "line-balance" | "oee";
...
const tools = [
  ...
  { id: "line-balance", href: "/tools/line-balance", step: "05", question: "工程の負荷は偏っていないか", title: "山積み表・ラインバランス", role: "作業配分を比較", badge: "実務ツール", message: "作業を移し、タクト超過の変化を見る", description: "工程別の作業時間を積み上げ、再配分前後の負荷を比較します。", features: ["積み上げ棒グラフ", "タクト超過", "配分の基準比較"], time: "3〜8分", level: "入門", preview: "balance" },
  { id: "oee", href: "/tools/oee", step: "06", question: "どのロスを改善するか", title: "OEE改善シミュレーター", role: "設備ロスを比較", badge: "実務ツール", message: "停止・速度・不良を、良品数につなげる", description: "OEEの内訳と改善シナリオから、推定良品生産量を比較します。", features: ["OEE自動計算", "3つのロス", "推定良品数"], time: "3〜5分", level: "入門", preview: "oee" },
] as const;
...
function MiniPreview({ type, title }: { type: string; title: string }) {
  return <div className={`tool-mini-preview tool-mini-preview--${type}`} onPointerDown={() => trackEvent("tool_preview_interaction", { tool_name: title, source_section: "tool_card", device_type: deviceType() })}>
    ...
    <p>{type === "gage" ? "誤差が増えると部品差が見えにくくなる" : type === "chart" ? "平均シフトを管理限界で検出" : type === "cpk" ? "中心がずれるとCpkが低下" : type === "doe" ? "線が交差すると交互作用あり" : type === "balance" ? "工程負荷とタクト超過を比較" : "改善条件から良品数を推定"}</p>
  </div>;
}
