...
function MiniPreview({ type, title }: { type: string; title: string }) {
  return <div className={`tool-mini-preview tool-mini-preview--${type}`} onPointerDown={() => trackEvent("tool_preview_interaction", { tool_name: title, source_section: "tool_card", device_type: deviceType() })}>
    <svg viewBox="0 0 320 142" role="img" aria-label={type === "inspection" ? "同じ合成基板画像をルールとAIで比較する模式図" : `${title}の動きを示すプレビュー`}>
      ...
      {type === "fabrication" && <><rect x="40" y="92" width="240" height="27" fill="#d8dfe4"/><path d="M40 92V68H110V92M180 92V68H280V92" fill="#81b8cf"/><path d="M40 68V48H110V68M180 68V48H280V68" fill="#e8c878"/><path d="M145 28V75m-7-7 7 7 7-7" fill="none" stroke="#1769aa" strokeWidth="2"/><text x="41" y="138">保護する膜</text><text x="181" y="138">窓から加工</text></>}
      {type === "pareto" && <><g fill="#1769aa"><rect x="45" y="55" width="40" height="65"/><rect x="105" y="80" width="40" height="40"/><rect x="165" y="95" width="40" height="25"/><rect x="225" y="105" width="40" height="15"/></g><polyline points="65,80 125,48 185,28 245,16" fill="none" stroke="#785518" strokeWidth="3"/></>}
      {type === "measurement" && <><path d="M60 34H268M112 72H216M138 110H190" stroke="#1769aa" strokeWidth="4"/><path d="M164 24V120" stroke="#737980" strokeDasharray="3 3"/><text x="28" y="137">少ない測定 ← → 狭い推定幅</text></>}
      {type === "confidence" && <><path d="M110 25V118" stroke="#60666d" strokeDasharray="4 4"/><path d="M218 25V118" stroke="#1769aa" strokeDasharray="2 3"/><path d="M64 55H280M140 92H244" stroke="#1d1d1f" strokeWidth="3"/><circle cx="172" cy="55" r="5" fill="#1769aa"/><circle cx="192" cy="92" r="5" fill="#1769aa"/><text x="86" y="136">差ゼロ</text><text x="215" y="136">ほしい幅</text></>}
      ...
      {type === "correlation" && <><g fill="#176b91"><circle cx="80" cy="86" r="5"/><circle cx="250" cy="95" r="5"/></g><g fill="#a15335"><rect x="75" y="34" width="10" height="10"/><rect x="245" y="44" width="10" height="10"/></g><text x="45" y="136">低温</text><text x="230" y="136">高温</text></>}
