...
import { journey, processSteps as thinFilmSteps, questions as thinFilmQuestions, processRelated, processCopy, PROCESS_VERSION } from '@/data/semiconductor-process';
...
import { assemblyCopy, assemblySteps, assemblyQuestions, assemblyRelated } from '@/data/semiconductor-assembly';
...
export function ProcessExplorer() {
  ...
  const processSteps = assembly ? assemblySteps : thinFilmSteps;
  const questions = assembly ? assemblyQuestions : thinFilmQuestions;
  ...
  return <div ref={root} className={styles.explorer}>
    ...
    <div className={styles.work} ref={work} tabIndex={-1}>
      {state.view==='overview' && <section className={styles.overview} aria-labelledby="process-overview-title">
        ...
        {state.overview===4 ? <><h3>{assemblyCopy.heading}</h3><p>{assemblyCopy.intro}</p><button type="button" className={styles.primary} onClick={() => navigate({type:'enter',experience:'assembly'})}>組み立てを体験する</button></> : state.overview===2 ? <button type="button" className={styles.primary} onClick={() => navigate({type:'enter'})}>一つの加工を拡大してみる <span aria-hidden="true">↗</span></button> : <button type="button" onClick={() => navigate({type:'overview',index:state.overview===5?2:state.overview+1})}>{state.overview===5?'ウエハ上の加工に戻る':'次の地点を見る →'}</button>}
      ...
      {state.view==='process' && <>
        ...
        <div className={styles.processHeader}><div><p className={styles.eyebrow}>{assembly?'後工程の一例':'前工程の一例'} / {state.step+1} OF {processSteps.length}</p><h2>{step.verb}</h2><p>{step.term} <span>· {state.progress===1?step.after:step.before}</span></p></div><button type="button" onClick={() => navigate({type:'overview',index:assembly?4:2})}>全体像に戻る</button></div>
        <div className={styles.lab}>
          <aside className={styles.locator}><Wafer marked/><p>{assembly?'★のチップを追います。素子・配線の形成と検査を終えた別の模式例です。':'ウエハの一部分の断面を見ています。実物を切断する操作ではありません。'}</p><nav className={styles.steps} aria-label={`加工の${processSteps.length}工程`}>{processSteps.map((s,i)=><button type="button" key={s.id} aria-current={state.step===i?'step':undefined} onClick={()=>navigate({type:'step',index:i})}><span>{i+1}</span>{s.term}{state.completed.includes(s.id)&&<small aria-label="確認済み">✓</small>}</button>)}</nav></aside>
          <div className={styles.stage}>
            {assembly ? <AssemblyDiagram step={assemblySteps[state.step].id} progress={state.progress} inside={inside} highlight={question}/> : <ProcessDiagram step={state.step} progress={state.progress} highlight={question}/>}
            ...
            <div className={styles.previousNext}><button type="button" disabled={state.step===0} onClick={()=>navigate({type:'step',index:state.step-1})}>← 前の工程</button>{state.step<processSteps.length-1?<button type="button" onClick={()=>navigate({type:'step',index:state.step+1,autoplay:true})}>次の工程 →</button>:<button type="button" className={styles.primary} disabled={!state.completed.includes(processSteps[processSteps.length-1].id)} onClick={()=>navigate({type:'summary'})}>{assembly?'組立のまとめへ →':'繰り返す意味と、その先へ →'}</button>}</div>
          </div>
        </div>
        </section>
      ...
      {state.view==='summary'&&assembly&&<section className={styles.summary}><p className={styles.eyebrow}>切り分け・組み立てのまとめ</p><h2>組立完了。次は最終検査</h2><AssemblyDiagram step="trim-form" progress={1} inside/><p>チップを固定する。電極を外へつなぐ。樹脂で保護する。それぞれ違う役割があります。</p><p>これはリードフレームとワイヤを使う一例です。組立を終えても、機能や性能を確かめる検査が必要です。</p><button type="button" className={styles.primary} onClick={()=>navigate({type:'overview',index:5})}>全体図で最終検査を見る →</button><button type="button" onClick={()=>navigate({type:'enter',experience:'assembly'})}>組み立てを最初から見直す</button><nav className={styles.related} aria-label="組立の理解を深める">{assemblyRelated.map(item=><Link href={item.href} key={item.id} onClick={()=>linkClick(item.id)}>{item.label} →</Link>)}</nav></section>}
      {state.view==='summary'&&!assembly&&<section className={styles.summary}><p className={styles.eyebrow}>この加工の先にあるもの</p><h2>同じことの繰り返しに見えても、作る場所と役割が違う。</h2><p>今見たのは「膜に形を作る一例」です。まだ電気的に動作するチップではありません。</p><CompletedStructure/><p className={styles.small}>別の完成構造の概念図です。直前の加工例がこの構造へ直接変わる様子ではありません。</p><p>実際には、場所・材料・目的を変え、熱処理なども含む多様な加工を重ねて素子と配線を作ります。この8工程だけを繰り返せば完成する、という意味ではありません。</p><h3>ウエハの加工が終わっても、まだ続く。</h3><p>電気的な検査、切り分け、外部との接続・保護、最終検査を経て製品になります。</p><button type="button" className={styles.primary} onClick={()=>navigate({type:'overview',index:3})}>全体図で、その先を見る →</button><button type="button" onClick={()=>navigate({type:'step',index:0})}>加工を最初から見直す</button><nav className={styles.related} aria-label="理解を深める">{processRelated.map(item=><Link href={item.href} key={item.id} onClick={()=>linkClick(item.id)}>{item.label} →</Link>)}</nav></section>}
    </div>
    ...
    <details className={styles.detail}><summary>この図で省略・簡略化していること</summary><p>{assembly?assemblyCopy.limits:processCopy.limits}</p></details>
  </div>;
}
