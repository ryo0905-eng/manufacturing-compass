...
export default function ControlChartPage(){return <main className="control-chart-page">
  ...
  <nav className="tool-related-links" aria-label="関連ツール"><span>関連ツール</span><Link href="/tools/yield-analysis">歩留まり低下を製品・装置で調べる →</Link><Link href="/tools/yield-dashboard">異常発見から確認実験まで体験 →</Link><Link href="/tools/oee">設備ロスの改善効果を試す →</Link><Link href="/guides/six-sigma">シックスシグマとDMAICの全体像 →</Link><Link href="/tools/cpk">工程能力を確認する →</Link><Link href="/tools/doe">実験計画法を学ぶ →</Link></nav>
<StatisticsCourseCta sourcePage="/tools/control-chart" />
  </main>}
