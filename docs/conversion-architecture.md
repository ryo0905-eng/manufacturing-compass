...
Cpk・工程比較の日本語結果から「次回の測定数を考える」へ接続し、計画後は改善の差の教材へ進む。共通tool_idはmeasurement-planner、コピー成功はmeasurement_plan_copied。既存共通イベントの定義を維持し、初期サンプルを成果に数えない。公開後14日の開始→結果→コピーを観測する。前提・投稿案・画像・配布URLは `docs/measurement-planner-spec.md`。本番反映・受信は未確認。
...
関連する企業研究ツール（2026-09-26）：工場新設記事の5案件一覧→2案件の段階・予定比較→出典確認／比較メモコピー／企業詳細・拠点マップへ接続。factory_project_compareのstartは最初の選択、compareは比較要求であり露出や人数ではない。copy_successはClipboard API成功。固定action・source_slug・ui_versionのみ送信。独立ページや直接広告は追加しない。本番反映後28日間の利用を観測し、本番反映・受信は未確認。[仕様](./factory-project-comparison-spec.md)。
...
`/tools` の `tool_finder` イベントは `ui_version=finder-v1`、`step`、任意の固定 `tool_id` のみ。`view` は入口見出しの露出、`start` は初回回答、`result` は操作後の案内見出しの露出で、それぞれマウント中1回。25%以上・前景タブの露出条件は既存 `observeVisibleOnce` を共有する。回答変更で未露出の旧結果の監視を解除する。`open` は主案内、`related` は補助リンクの明示クリック、`reset` はやり直しの都度記録する。回答一式・選択値・自由記述は送らない。初期表示を開始・結果に数えない。

`open` はクリック数、`result` はマウント中の初回露出数であり、単純な比率はユニーク転換率ではない。同じ画面での複数クリック・再選択を含み得る。計算完了や利用者の課題解決とは区別し、遷移先の既存イベントも別集計で確認する。本番受信は未確認。本番反映後14日から観測し、母数が少なければ需要未検証として延長する。
...
2026-09-26：1件／2件切替を同じ入口に追加。比較モードは `job_posting_comparison`、ui_version=compare-v1、固定actionのみで観測する。view/start/resultはモードごとにマウント中1回。既存1件モードのイベントは維持し、件数を合算しない。回答を引き継いで比較する場合は比較モードのstartを経ずresultになることがある。本番反映後28日間、2件モードの結果露出とコピーを確認する。既存の年収記事→確認ノート→相談メモの導線を維持。詳細は確認ノート仕様を参照。本番反映・受信は未確認。
