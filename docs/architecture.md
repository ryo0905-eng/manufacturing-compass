...
### 薄膜加工・配線とCMP・組立の仕事紹介（2026-09-22）

`semiconductor-work.ts`に役割・架空事例・出典・更新日・関連記事を定義。`WorkRolePanel.tsx`は選択された役割を受け取る静的なSVGと文章の表示部。選択状態とページ内の表示計測抑制はProcessExplorerに置き、加工の状態遷移へ混ぜない。`workLessons`が体験IDごとの役割・出典・版を選ぶ。画面移動で選択を解除し、表示済みキー（体験ID＋役割ID）は保持する。薄膜と組立の図は表示部内で描き分け、配線とCMPの図は`InterconnectWorkGraphic.tsx`に分離。Serverページにも3体験の各3役割の文章と出典を出力。URL、canonical、sitemapの日付（同日更新）、既存6体験の計測版と再生動作を維持し、仕事紹介のイベントは別版で識別する。新規依存・API・保存は追加しない。
