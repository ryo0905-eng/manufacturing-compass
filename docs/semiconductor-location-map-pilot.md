...
この文書は初期候補の調査過程と、公式情報で確認した10社・24拠点の根拠を記録します。正式な拠点名、所在地、稼働状態、半導体との関係、職種、採用状態を企業公式情報で確認できた項目だけ、公開用の静的データへ移します。
...
5社・13物理拠点を企業公式情報で確認しました。正式拠点名、住所、関係法人、半導体との関係、拠点種別、稼働状態の根拠を確認できており、座標を使わないV0では `complete` として扱えます。座標確認はV1の詳細ピンを公開する場合だけ必要です。
...
- 24拠点は座標を使わないV0の公開基準を満たした。座標と二者レビューはV1の詳細ピンへ進む場合に行う
...
- 全国ハブと既存 `/companies` への拠点情報再利用は有望。V0公開前に出典の到達確認と表記レビューを行い、座標付与は求めない
...
| 座標・精度 | V1のみ必須 | 施設、地番、住居番号、町字、市区町村、都道府県の粒度と、取得方法、座標ソース、レビュー状態を記録する。V0には含めない |
...
- 稼働中と計画中を区別している
- V1へ進む場合に使う人手確認時間の計測方法を記録している
- 座標を使わないV0へ進み、正確な施設ピンは利用確認後のV1で判断すると記録している
