...
10社から代表1拠点ずつ選び、企業公式住所を住所検索へ入力しました。座標はデータ構造と精度管理を検証するための暫定値で、まだ人による地図上の確認を完了していません。

住所検索にはデジタル庁アドレス・ベース・レジストリ（ABR）を利用する全国Q地図APIを使いました。同APIが市区町村代表点しか返さなかった拠点は、その値を採用せず、企業公式の地図リンクまたはOpenStreetMapの名称付き施設を補助照合に使いました。

| 拠点ID | 緯度 | 経度 | 精度 | 取得・照合方法 | レビュー状態 | 座標確認リンク |
| --- | ---: | ---: | --- | --- | --- | --- |
| `tsmc-jasm-kumamoto` | 32.886088 | 130.843565 | `parcel` | 公式住所をABR地番へ完全一致 | 自動照合のみ | [全国Q地図](https://qchizu.jp/maps/#17/32.886088/130.843565/) |
| `micron-hiroshima` | 34.384981 | 132.678461 | `residential-detail` | 公式住所をABR住居番号へ完全一致 | 自動照合のみ | [全国Q地図](https://qchizu.jp/maps/#17/34.384981/132.678461/) |
| `tel-miyagi-taiwa` | 38.366095 | 140.855871 | `parcel` | 公式住所をABR地番へ一致。表記正規化によりスコア0.95 | 自動照合のみ | [全国Q地図](https://qchizu.jp/maps/#17/38.366095/140.855871/) |
| `kioxia-yokkaichi` | 35.019584 | 136.612247 | `facility` | ABRは市代表点のため不採用。名称付き工場区域の中心を補助照合 | 自動照合のみ | [OpenStreetMap](https://www.openstreetmap.org/relation/8866755) |
| `screen-hikone` | 35.232462 | 136.265452 | `facility` | ABRは市代表点のため不採用。名称付き事業所区域の中心を補助照合 | 自動照合のみ | [OpenStreetMap](https://www.openstreetmap.org/way/641965677) |
| `advantest-gunma-rd` | 36.208072 | 139.491423 | `parcel` | 公式住所をABR地番へ完全一致 | 自動照合のみ | [全国Q地図](https://qchizu.jp/maps/#17/36.208072/139.491423/) |
| `renesas-takasaki` | 36.331604 | 139.070079 | `facility` | ルネサス公式拠点ページの地図リンク | 自動照合のみ | [公式地図リンク](https://goo.gl/maps/98pi4oEyaX5KGuaEA) |
| `rohm-kyoto-hq-factory` | 34.994812 | 135.728840 | `facility` | ABRは市代表点のため不採用。名称付き企業地点と住所検索を相互照合 | 自動照合のみ | [OpenStreetMap](https://www.openstreetmap.org/node/12162125058) |
| `socionext-shin-yokohama` | 35.507306 | 139.611503 | `facility` | ソシオネクスト公式拠点ページの地図リンク | 自動照合のみ | [公式地図リンク](https://maps.app.goo.gl/cZf492Mjk6K2feaz9) |
| `sumco-imari-nagahama` | 33.282370 | 129.850366 | `parcel` | 公式住所をABR地番へ完全一致 | 自動照合のみ | [全国Q地図](https://qchizu.jp/maps/#17/33.282370/129.850366/) |

住所データの帰属表示: デジタル庁 アドレス・ベース・レジストリ。住所検索・確認リンク: 全国Q地図。施設名による補助照合: OpenStreetMap contributors。

### 座標パイロットの結果

- 10件中5件は地番または住居番号レベルの座標を取得できた
- 5件はABRの座標が市区町村代表点だったため、その値を工場ピンとして採用しなかった
- 5件のうち2件は企業公式地図リンク、3件は名称付き施設情報で暫定補完した
- `address` だけでは精度差を表せないため、座標精度を `facility`、`parcel`、`residential-detail`、`machiaza`、`municipality`、`prefecture` に分ける
- 座標の取得方法を `official-map`、`address-geocode`、`named-facility` に分ける
- 全10件を `automated-check` とし、人が公式住所、地図、周辺施設を確認するまで `human-reviewed` にしない
- 全国Q地図APIの結果を保持する場合は確認リンクと帰属表示が必要で、別地図へ表示する場合は同APIの利用条件を満たせない可能性がある。本番値としてはそのまま採用しない
- OpenStreetMapの施設情報もライセンス条件の確認が必要なため、今回の3件は位置確認用の暫定値に限定する

### 本番データへ移す条件

1. 公式住所と座標が同じ施設を示すことを人が地図上で確認する
2. 座標が敷地中心、入口、本社建屋などのどこを指すかを確認する
3. 使用する地図基盤で保存・表示可能な座標ソースか確認する
4. 6桁程度へ丸め、必要以上に細かな測量精度を示さない
5. レビュー担当、レビュー日、座標ソースを記録する

座標生成の参考資料:

- [ABRジオコーダー](https://lp.geocoder.address-br.digital.go.jp/)
- [全国Q地図 住所・所在地検索API](https://www.qchizu.jp/geocoder-api/)
...
| 座標・精度 | 必須 | 施設、地番、住居番号、町字、市区町村、都道府県の粒度と、取得方法、座標ソース、レビュー状態を記録する |
