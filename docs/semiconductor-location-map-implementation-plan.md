...
有限イベント型と各UIからの送信、上記を対象にしたE2Eを追加しました。検索語そのものは送らず、検索利用時は結果件数だけを送ります。`npm run typecheck` は通過しました。ローカル開発サーバーを使わない公開URL向け実行にも対応し、2026-09-05にDesktop Chrome 7件、Pixel 7 7件の合計14件がすべて通過しました。
...
2026-09-05時点で、データ、表示、SEO、内部導線、計測コードは上記基準を満たしています。公開URL上のDesktop Chrome・Pixel 7向けE2Eも合計14件通過したため、座標を使わない全国V0の公開ゲートは完了です。
