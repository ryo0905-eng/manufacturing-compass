...
この文書はV1で正確な拠点ピンを追加する時の確認台帳です。公開V0は座標を使わないため、このレビューを公開条件にしません。正式住所、座標、出典の正本はパイロット台帳とし、レビュー結果を反映した後に `human-reviewed` へ変更します。
...
正確な拠点ピンを追加するV1は、座標レビューに加えて次を満たした場合にGoとします。

- V1で最初に表示する5拠点以上がすべて `human-reviewed` である
- 拠点の存在、稼働状態、求人状態を別の更新周期で管理できる
- 全国ページの初期HTMLに、地図なしでも価値のある拠点一覧と説明を出せる
- `/companies` と `/industry-map` から自然に接続できる
- 求人連携やリアルタイム求人数をV1へ含めない

現時点の判定は `V0: Go`、`V1: 条件付きGo` です。V0は公式確認済み10社・24拠点を都道府県選択図と一覧で公開し、座標を使いません。V1は座標レビューと公開ピンの品質確認後に判断します。
...
3. 実測した中央値から、利用の多い拠点へ詳細ピンを追加する工数を見積もる
4. V0の利用データと合わせてV1へ進むか判断する
5. V1がGoの場合だけLeaflet、地理院タイル、座標データを実装する
