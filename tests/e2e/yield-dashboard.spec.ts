...
test.describe("歩留まり原因調査ダッシュボード", () => {
  test("ガイド付き調査を確認実験まで完了できる", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /変更前 F26/ }).click();
    await page.getByRole("button", { name: /低下後 F26/ }).click();
  ...
  test("自由探索のリセットとスマートフォン表示が機能する", async ({ page }) => {
    ...
    await expect(filterState.getByText("24ロット", { exact: true })).toBeVisible();
    await page.getByRole("button", { name: "全条件をリセット" }).first().click();
    await expect(filterState.getByText("56ロット", { exact: true })).toBeVisible();
