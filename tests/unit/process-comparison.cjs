...
function load(file, dependencies = {}, globals = {}) {
  ...
  vm.runInNewContext(code, { exports, require: name => { if (name === '@/data/improvement-report') return load('src/data/improvement-report.ts'); if (name === '@/components/ImprovementReportEditor') return { ImprovementReportEditor: 'ReportEditor', reportAction() {} }; if (name === '@/lib/confirm-input-replacement') return load('src/lib/confirm-input-replacement.ts', {}, { window: { confirm: () => true } }); if (name === '@/components/ToolWorkspaceFile') return { ToolWorkspaceFile: 'WorkspaceFile' }; if (name === '@/components/PracticalToolNextSteps') return { PracticalToolNextSteps: 'NextSteps' }; if (name === '@/lib/use-practical-tool-journey') return load('src/lib/use-practical-tool-journey.ts', { ...dependencies, react: { ...dependencies.react, useEffect() {} }, '@/lib/observe-visible': { observeVisibleOnce() {} } }); if (name === '@/data/practical-tool-text') return load('src/data/practical-tool-text.ts'); if (!(name in dependencies)) throw Error(name); return dependencies[name]; }, ...globals }, { filename: file });
  return exports;
}
